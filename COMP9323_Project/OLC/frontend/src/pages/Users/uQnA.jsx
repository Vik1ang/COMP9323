...
const UQnA =()=> {
    ...
    const goArticle=(id)=>{
        history.push(`/userArticle/${community}/QnA/${id}`);
    };
    const goAdd=()=>{
        history.push(`/userAdd/${community}/QnA`);
    };

    const getPostByCommunity = async () => {
        const loggedinToken = localStorage.getItem('token');
        await fetch(`${URL}/post/getAllPosts?community=${community}&type=2&sort=${sortIndex.indexOf(sortBy)}`, {
          method: 'GET',
          headers: {
                ...
                <div className="container-xl">
                    ...
                    <div class="row-xl">
                        ...
                        <button class="btn btn-default btn-full" style={{marginTop:20,marginBottom:20}}>
                            <img src={add} class="wave" alt="wave" /><span className="smallText" onClick={goAdd}>Add New Question</span>
                        </button>
                    </div>
